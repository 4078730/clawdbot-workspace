...
### 仕事
- **FY26事業計画** — 3/7〆切分（β版定義書・具体感の図・体制図・CCドットスケール）完了 ✅ 3/4。残: 申請書更新・Confluence更新・リソース不足明記（3/10〆切）
- **FY26ターゲットタスク選定** — 戦略ドキュメント作成済み(`Projects/rfa/fy26-task-selection-strategy.md`)
- **IndexVLA論文執筆** — 継続中
- **彩都パーツセンターPoC** — オートストア出庫作業。3/25 現場ディスカッション予定
- **Microsoft協業** — ADS Day 1/2完了(2/19-20)、LeRobot+Azure推論確定。Houston出張 3/12〜3/18（ホテル未予約 ⚠️）
- **神戸工場キッティング工程** — VLA適用候補、動画データ解析中
- **TRLC-DK1-X × 2セット購入** — ✅ キャンセル済み（2026-03-04 確認）
- **GELLO発注** — XC330-M288-T×18他、Robotis見積もり回答待ち（2/25依頼済み）
...
### 2026-03-07（FY26事業計画 3/10〆切分 完了 + Houston出張準備）
- 申請書更新（ヒューマノイド削除・経費精査）✅、Confluence更新（リソース不足明記）✅、Houstonホテル予約 ✅
- **確定した方針**: β版（9月末）= CA相当・研究室実証・成功率90%/リトライ込95%、公式リリース（3月末）= QMS全項目・TPL-QMS準拠
- **彩都現場実証**: 梱包工程が有力候補（3/25現場ディスカッション予定）
- **Houston出張（3/12〜3/18）緊急**: パスポート更新（3/9最優先）、ESTA承認待ち（H39T9Z2127M1515Z）、EPOCH渡航申請・SFO/Millbrae要否確認（3/10期限）
- 人員バイネーム（confluence-review-v2.md 3.4体制）はkonuki作業待ち
- Agentic（×Agentive）に表記統一確定

### 2026-03-04（FY26事業計画 3/7〆切分 完了）
- β版定義書（機能単位）、「具体感が伝わる図」、体制図（時間軸）、CCドットスケール追加 — すべて完了 ✅
- β版KPI: 成功率90%/リトライ込95%、彩都梱包工程が有力
- メンバー追加アサイン確定（Zhu・黒川）✅

### 2026-02-26（FY26体制MTG：α→β呼称変更・定義軸変更・体制整理）
- **呼称変更（重要）**: 「α版」→「β版」に統一。RoboSyncはCA/GAを設けずGAとして出す文化のため、従来の「α版」（ピック→作業台配置・VLAスキルブロック）はRoboSync定義では「β版」相当。
- **定義の軸変更**: タスク基準 → **機能単位**に変更。RoboSyncの機能ブロック（VLA Policy・DataOps・安全制御など）単位でα/β/GAをマトリックスで整理する方向。
- **体制: 時間軸で分ける**: 上期（〜9月）= 全員でβ版リリース全力（小栗・Zhu・黒川もQ2末まで参加）、下期（9月〜）= スキル拡張・エージェント（村上・加賀屋先行）
- **MS統合**: 別扱いせず、RoboSync統合の軸として一本化。MS 5月頃終了後に全員でRoboSyncリリースに合流。
- **CCドットスケール**: POCと商用版は別物。MS終了後（5〜9月）に商用基盤への載せ替え必要。石丸さんがバイウィークリーでレビュアー参加。
- **ヒューマノイド**: 申請書から削除・最小化。部活動で探索、9月以降に起案可。
- **主要成果物（3/10提出）**: β版定義書（機能単位）、「作るものの具体感が伝わる図」（大坪さんリクエスト）、体制図（時間軸形式）
- 詳細: `memory/2026-02-26.md`

### 2026-02-25（GELLO発注・MS体制確定・CTL応募）
- **GELLO発注確定（UR5e双腕テレオペ、佐江戸2セット）**: XC330-M288-T×24、M077×4他。牧さんへ依頼済み、Robotis見積もり待ち。詳細: `memory/2026-02-25.md`
- **MS協業チーム拡張確定**: ZhuさんとKurokawaをMS協業メンバーとして正式追加（2/25 MS Weekly Sync）
- **LLM-Robotics勉強会 CTL応募**: 大坪さんリクエスト、奥村さんと共同応募。konukiのスタンス=ゆるい技術共有継続希望、オフラインキックオフ・成果報告不要と伝達済み。締切3/23
